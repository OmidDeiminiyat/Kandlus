const UserCode = getCookie('Kandlus-users');
  ...
  .then(data => {
    console.log(data);
      ...
      const testArray = convertStringToArray(data[0].savedArticle);
...
<figure class="mb-15 artImg">
    <a href="single.html?id=${post.seo}">
        <img src="../assets/images/Articles/width/${post.image}" alt="${post.categoryOne}, ${post.categoryTwo}, ${post.tagOne}, ${post.tagTwo}, ${post.tagThree}" >
    </a>
</figure>
<a href="single.html?id=${post.seo} ">${post.title}</a>
<div class="entry-meta meta-2 font-x-small color-muted">
    <span class="mr-10"> <i class="ti-time"></i>${post.created_at} </span>
